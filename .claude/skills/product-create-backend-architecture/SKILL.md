---
name: product-create-backend-architecture
description: Create architecture manifest for a backend service - declares language, type, conventions and modules
...
Step 3: Pick service type and language
        --- GATE B: user picks type + language ---
...
- `overview.md` — service-specific: purpose, domain summary
...
- Create frontend architectures — use `/product-create-frontend-architecture`
...
1. **MANDATORY CONFIRMATION GATES.** This skill has six confirmation gates labeled **GATE A through GATE F**. Each gate is a hard stop. At each gate you MUST:
   ...
   **Gate inventory:**
   ...
   - **GATE B**: post Step 3. User picks service type + language.
...
2. **All user-facing text in Spanish.** Without exception. This includes step headings, lists, tables, AskUserQuestion labels and descriptions, status updates, and any narrative text. The SKILL is written in English (instructions for the agent), but every word the user sees is in Spanish. If you catch yourself about to write English to the user, stop and translate.

3. **Use display_name, not id or filename.** When showing conventions to the user, use the `display_name` from the convention's frontmatter (e.g., "Servidor HTTP (Fastify)"), not the id (`http-server`) or filename (`http-server.md`). The id is internal and only goes into the manifest.
...
11. **Custom conventions mirror the catalog structure.** When generating a custom convention, do NOT invent the document structure. If a catalog convention exists with the same id (replacement case), use it as the structural template: same H2 sections in the same order, same rule count where possible. Adapt code examples and content to the user's chosen package, but keep section headings, the Rules list semantics, and the integration notes consistent. If the catalog has a `## Structure` section, the custom MUST have it. If the catalog has a `## REST conventions` table, the custom MUST have it. The rule numbering and intent from the catalog are preserved — code changes, contracts do not.
...
```markdown
Necesito el nombre del servicio.

**Uso:** `/product-create-backend-architecture {nombre-servicio}`

**Ejemplos:**
- `/product-create-backend-architecture user-service`
- `/product-create-backend-architecture api-gateway`
- `/product-create-backend-architecture notification-worker`
```
...
4. Read `.claude/conventions/index.md` and the frontmatter of every convention file under `.claude/conventions/{language}/` for each supported language. Keep id, display_name, description, applies_to, required_by, package in memory.

Do NOT confront the PRD against the catalog in this step. That happens later (Step 4).

### Step 2: Validate Service Scope from PRD

Goal: confirm what the PRD says about this specific service. Do not show conflicts with the catalog yet.

From the PRD architecture and requirements, extract what is mentioned about `{service-name}` (if anything): purpose, technologies, integrations, persistence.
...
- **Persistencia:** {db mentioned, or "No especificada"}
- **Integraciones:** {externals mentioned, or "Ninguna"}
...
**=== GATE B — Service type + language ===**

Render two `AskUserQuestion` calls in a single message:

**Question 1 — "Que tipo de servicio es?"**
Options:
- `API` / "Servidor HTTP que expone endpoints REST"
- `Worker` / "Procesa jobs o mensajes en segundo plano"
- `CLI` / "Herramienta de linea de comandos"
- `Library` / "Paquete reutilizable, sin runtime propio"

**Question 2 — "En que lenguaje?"**
Options: one per language folder under `.claude/conventions/`. Label = language name capitalized. Description = first sentence from that language's `_base.md` description.

Suggest defaults if the PRD mentioned them, but always ask.

**STOP and wait for both answers. Do NOT proceed to Step 4 until the user has answered both questions.**

Store as `service_type` and `language`.
...
- Filter remaining conventions by `applies_to` containing `service_type`
- Exclude conventions that exist primarily as auto-includes (those whose `applies_to` is broad but that have `required_by` populated and are not typically picked directly). For the current Node catalog, this means: suggest `http-server`, `validation`, `logging` for `api`; `logging` for `worker` and `cli`; nothing extra for `library`.
...
Para un servicio **{service_type}** en **{language}**, te sugiero estas convenciones del catalogo:
...
- **Replace a catalog convention with a different package** (e.g., "quiero Express en vez de Fastify"): open the **assisted custom creation sub-flow (4.3)** for that concern. The catalog convention is removed from the set; the custom replaces it with the same id.
- **Add a new concern not in the catalog** (e.g., "necesito una convencion para generar PDFs"): open the **assisted custom creation sub-flow (4.3)** with a new id.
...
- If replacing a catalog one: keep the same id (e.g., `http-server`).
...
1. **Que paquete o framework queres usar?** (ej. Express, Joi, Winston, etc.)
...
- Use `WebFetch` on the most relevant pages (getting started, best practices, error handling, integration patterns).
...
**Reglas:**
{render each rule from the catalog file, renumbered if needed, with a short note when the wording was adapted for the new package — for example "(adaptada: ahora menciona Express)"}

Si alguna regla del catalogo no aplica al nuevo paquete, la marco como N/A explicando por que (sin removerla).
...
This gate fires **once per custom convention** being created. If the user is creating three customs in this flow, GATE D opens three times (one per convention).
...
- **Body**: every mandatory section from the template, plus any optional section decided in 4.3.3. Headings exactly as the template prescribes (`## When to use`, `## Package`, `## How to use`, `## Rules`, etc.).
...
- Content in English (matches the catalog), like the rest of `.claude/conventions/`. The `display_name` is in Spanish.
...
2. **Modulos del dominio** — Lista separada por comas. Ejemplos: users, sessions, orders.
3. **Algo mas que quieras dejar anotado?** (deployment, integraciones externas, restricciones). Si no, escribi "no".
