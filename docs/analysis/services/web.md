...
los permisos viven en `api`. El frontend consume la REST API y refleja lo que
ésta responde.
...
| Tipo | Target | Detalle |
|---|---|---|
| REST API | `api` | Toda la funcionalidad. `REACT_APP_API_URL`, default `http://localhost:8080` |
...
1. **`web` → `api`**: única integración de datos. REST con JWT Bearer.
...
2. **`web` → Google**: obtención del token OAuth en el cliente, que luego se
...
[`docs/analysis/ux/web/`](../ux/web/index.md): stack de UI,
...
| Documento | Path |
|---|---|
| Arquitectura | `docs/architectures/web/` |
| Relevamiento UX | `docs/analysis/ux/web/` |
