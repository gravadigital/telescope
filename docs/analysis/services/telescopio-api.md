# Análisis de Servicio: telescopio-api
...
| | |
|---|---|
| **Nombre** | `telescopio-api` |
| **Módulo Go** | `github.com/gravadigital/telescopio-api` |
...
El frontend (`telescopio-web`) no tiene lógica de negocio propia: consume esta API.
...
1. **`telescopio-web` → `telescopio-api`**: toda la interacción del usuario.
   El frontend consume la REST API con JWT en el header `Authorization`.
2. **`telescopio-api` → Google OAuth**: verificación del token de identidad
   durante login/registro con Google.
3. **`telescopio-api` → SMTP**: emails disparados por cambio de etapa,
   cancelación de evento y solicitud de reset de password.
4. **`telescopio-api` → MinIO**: persistencia de las propuestas cuando
...
| Documento | Path |
|---|---|
| Arquitectura | `docs/architectures/telescopio-api/` |
| API Specification | `docs/apis/telescopio-api.yaml` |
